...
function DmdOptions (options) {
  ...
  // load the '--configure' options if any
  let configureOptions = {}
  if (options.configure) {
    const configrue = options.configure
    const configureLc = configure.toLowerCase()
    if (configureLc.endsWith('.json')) {
      const configureContents = fs.readFileSync(options.configure, { encoding: 'utf8' })
      configureOptions = JSON.parse(configureContents)
    }
    else if (configureLc.endsWith('.js') || configureLc.endsWith('.cjs')) {
      configureOptions = require(configure)
    }

    const unsupportedOptions = ['recurseDepth', 'source', 'sourceType', 'tags']
    for (const unsupportedOption of unsupportedOptions) {
      if (unsupportedOption in configureOptions) {
        console.warn("Config file option '" + unsupportedOption + "' is not currently supported and will be ignored.")
      }
    }
  }
  ...
  */
   ...
   * @type {array}
   */
  this.plugin = arrayify(options.plugin).concat(arrayify(configureOptions.plugins))
